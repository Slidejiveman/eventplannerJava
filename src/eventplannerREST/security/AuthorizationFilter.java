...
public class AuthorizationFilter implements ContainerRequestFilter{
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        List<EmployeeRole> classRoles = extractRoles(resourceClass);
        
        // Get the resource method which matches with the requested URL
        // Extract the roles declared by it
        Method resourceMethod = resourceInfo.getResourceMethod();
        List<EmployeeRole> methodRoles = extractRoles(resourceMethod);
	...
	private List<EmployeeRole> extractRoles(AnnotatedElement annotatedElement) {
		if (annotatedElement == null) {
			return new ArrayList<EmployeeRole>();
		} else {
			Secured secured = annotatedElement.getAnnotation(Secured.class);
		    if (secured == null) {
		    	return new ArrayList<EmployeeRole>();
		    } else {
		    	EmployeeRole[] allowedRoles = secured.value();
		    	return Arrays.asList(allowedRoles);
		    }
		}
	}

	private void checkPermissions(List<EmployeeRole> allowedRoles, String username) throws Exception {
		...
		for (EmployeeRole role : allowedRoles) {
			if (user.isAuthorized(role)) return;
		}
		throw new Exception();
	}
}
